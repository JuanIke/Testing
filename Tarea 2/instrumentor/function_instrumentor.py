...
class FunctionInstrumentor(NodeTransformer):
    ...
    def visit_FunctionDef(self, node: FunctionDef):
        ...
        if isinstance(transformedNode.body[-1], Return):
            original_return_value = transformedNode.body[-1].value
            profiler_call = Call(
                func=Attribute(
                    value=Name(id='Profiler', ctx=Load()),
                    attr='record_end',
                    ctx=Load()),
                args=[Constant(value=transformedNode.name), original_return_value],
                keywords=[])
            transformedNode.body[-1] = Return(value=profiler_call)
        else:
